...
def changeChannelName(databaseValues, name, id):
    '''
    This function changes the name connected to a channel id
    :param databaseValues: configuration values for the database
    ...
    '''
    try:
        conn = pymysql.connect(user=databaseValues['user'], host=databaseValues['host'], password=databaseValues['password'], database=databaseValues['name'])
        cursor = conn.cursor()
...
def resetChannel(databaseValues, id):
    '''
...
def createDatabase(databaseValues):
    '''
    ...
    '''
    try:
        conn = pymysql.connect(user=databaseValues['user'], host=databaseValues['host'], password=databaseValues['password'])  # Establish mysql connection
        cursor = conn.cursor()
...
def dropDatabase(databaseValues):
    ...
    :param databaseValues: a dictionary containing configurations for the database to be deleted
    Example: {'user': 'root', 'host':'127.0.0.1', 'password': '1234', 'name': 'EmployeeDatabase'}
    :return: True if the database was deleted, False otherwise
    '''
    try:
        conn = pymysql.connect(user=databaseValues['user'], host=databaseValues['host'], password=databaseValues['password'])
        cursor = conn.cursor()
        sql = "drop database if exists %s" % (databaseValues['name'],)
        cursor.execute(sql)
        conn.commit()
        conn.close()
        return True
...
def idExists(id, databaseValues):
    '''
    This function checks if a channel id exists in the database
    :param id: channel id to look for 
    :param databaseValues: a dictionary containing mysql connection configurations. 
    Example: {'user': 'root', 'host':'127.0.0.1', 'password': '1234', 'name': 'EmployeeDatabase'}
    :return: True if the channel id was found, false otherwise
    '''
    try:
        conn = pymysql.connect(user=databaseValues['user'], host=databaseValues['host'], password=databaseValues['password'], database=databaseValues['name'])      #Establish connection to database
        cursor = conn.cursor()
...
def readFromDatabase(databaseValues, readParameters):
    '''
    This funtion reads a specified databse using specified parameters
    :param databaseValues: a dictionary containing mysql database configurations
    Example: {'user': 'root', 'host':'127.0.0.1', 'password': '1234', 'name': 'EmployeeDatabase'}
...
def addToDatabase(databaseValues, listOfItems):
    '''
    This function adds entries to a specified database using a provided list of entry values
    :param databaseValues: a dictionary containing mysql configurations
    Example: {'user': 'root', 'host':'127.0.0.1', 'password': '1234', 'name': 'EmployeeDatabase'}
    :param listOfItems: a dictionary containing containing the needed values to make an entry to the database
    Example: {1: [ 25.6, 'Kg', '20'], 2: [22.3, 'Celsius', '2']}
    :return: True if something was added to the database, false otherwise
    '''
    try:
        conn = pymysql.connect(user=databaseValues['user'], host=databaseValues['host'], password=databaseValues['password'], database=databaseValues['name'])        #Connects to the database
        curs = conn.cursor()
        ...
        for index in listOfItems:                                                 #For each item in the list, gets the values to be added to the database
            id = index
            list = listOfItems[index]
